...
public class UseReduceProduit
	...
	private static int version1(int[] tab)
		{
		BinaryOperator<Integer> operatorProduct = new BinaryOperator<Integer>()
			{
		...
		return NumberManipulator.reduce(tab, operatorProduct, 1);
		}
	...
	private static int version2(int[] tab)
		{
		BinaryOperator<Integer> operatorProduct = (t, u) -> t * u;
		return NumberManipulator.reduce(tab, operatorProduct, 1);
		}
